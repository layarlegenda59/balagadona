...
export const getMockCoords = (address: string, distanceLabel: string) => {
  ...
  if (distanceLabel.includes('3 - 5')) distFactor = 4
  else if (distanceLabel.includes('5 - 10')) distFactor = 7.5
  else if (distanceLabel.includes('> 10')) distFactor = 12
  ...
  persist(
    (set, get) => ({
      ...
      optimizeBatchSequence: (batchId) => {
        ...
        const unvisited = [...batchOrders].map((order) => {
        ...
        try {
          ...
          if (!bError) {
            if (dbBatches && dbBatches.length > 0) {
              ...
              // Self-healing: if database is empty, seed default batches to Supabase
              await supabase.from('batches').insert(DEFAULT_BATCHES)
              set({ batches: DEFAULT_BATCHES })
            }
          }
        } catch (err) {
